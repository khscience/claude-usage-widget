...
- 进度条 = ccusage 算的已用**费用** ÷ 你设定的费用限额；颜色随占比变（蓝 <70% / 黄 <90% / 红 ≥90%）
- 鼠标悬停看 tooltip（完整费用、限额、重置时刻）
- 5h 显示当前活跃块，空闲时显示「空闲」
...
| 刷新间隔 | 5–3600 秒，默认 10 |
| 不透明度 | 40%–100% 滑块，实时预览 |
| 5h 费用限额 | 进度条分母（USD），默认 $34.25 |
| 周费用限额 | 进度条分母（USD），默认 $330.61 |
| 周窗口重置 | 与你 `/usage` 显示的 "Resets …" 对齐（默认周三 19:00）|
| ccusage 命令 | 默认 `npx -y ccusage`；装了全局版可填 `ccusage` |
| 开机自动启动 | 写 `HKCU\...\Run`，无需管理员权限 |

> **校准方法**：默认值是按 Pro 订阅校准的，多数人开箱可用。如想更精确，打开 Claude Code 输入 `/usage` 看官方百分比，用「**当前费用 ÷ 官方百分比**」算出真实限额填进设置即可（费用见悬浮窗 tooltip）。
>
> 为什么用费用而不是 token？因为 ccusage 的 `totalTokens` 把 cache read 按全额算（实测 cache 占 97%），而官方限流给 cache read 权重低得多，导致 token 算的占比会严重失真和漂移。费用天然按真实定价加权（cache read 打 0.1 折），更稳。
